...
<template>
  <main v-if="data" class="shell">
    ...
    <section class="panel table-panel signal-panel"><div class="section-title"><div><p class="eyebrow">RISK SIGNALS</p><h2>风险信号灯</h2><small>{{ indicators.length }} 项指标</small></div><div class="tabs"><button v-for="group in groups" :key="group" :class="{ active: activeGroup === group }" @click="activeGroup = group">{{ group }}</button></div></div><div class="signal-grid"><article v-for="item in indicators" :key="item.id" class="signal-card"><div class="signal-main"><div class="signal-name"><b>{{ item.name }}</b><small>{{ item.id }} · {{ item.group }}<template v-if="item.source"> · {{ item.source }}</template></small></div><strong v-if="item.status === 'success'">{{ number(item.value!) }}{{ item.unit }}</strong><strong v-else>—</strong><span class="badge" :class="item.signal"><i></i>{{ label[item.signal] }}</span></div><div class="signal-meta"><span>分位 <b>{{ item.percentile === null ? '—' : item.percentile + '%' }}</b></span><span>观测 <b>{{ item.observationDate }}</b></span><span>{{ item.frequency }}</span><em v-if="item.change">{{ item.change }}</em></div><div class="percentile-track"><i :class="item.signal" :style="{ width: (item.percentile ?? 0) + '%' }"></i></div></article></div></section>
